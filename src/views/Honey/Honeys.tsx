...
function createData(name: string, price: number, weight: number): Data {
  return {
    name,
    price,
    weight,
...
const rows = [
  createData('Acacia', 5600, 1),
  createData('Medicinal', 12000, 1),
  createData('Linden', 8341, 1),
  createData('Nectar', 3752, 1),
  createData('Canola', 9351, 1),
];
...
export default function Honeys() {
  ...
  const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const newSelected = rows.map((n) => n.name.toString());
